...
CREATE TABLE IF NOT EXISTS orders (
  ...
  -- 客户基础信息扩展（教务端录入）
  customer_name            VARCHAR(128) NULL COMMENT '客户姓名（教务端交付资料维护）',
  ...
  fund_remark              TEXT         NULL COMMENT '基金信息备注',
  registration_status      VARCHAR(32)  NOT NULL DEFAULT 'pending' COMMENT '个人信息登记表状态：pending待收齐/partial部分收齐/collected已收齐',
  author_registration_url  VARCHAR(500) NULL COMMENT '作者登记表附件URL',
  author_registration_name VARCHAR(255) NULL COMMENT '作者登记表附件原文件名',
  backup_submission_url    VARCHAR(500) NULL COMMENT '备用投稿信息表附件URL',
  backup_submission_name   VARCHAR(255) NULL COMMENT '备用投稿信息表附件原文件名',
  operation_method         VARCHAR(32)  NULL COMMENT '操作方式：一稿一投/两稿两投/三稿三投',
  ...
  backup_teacher           VARCHAR(128) NULL COMMENT '备用老师',
  ...
  sales_contact            VARCHAR(128) NULL COMMENT '对接销售',
  academic_owner           VARCHAR(128) NULL COMMENT '负责教务',
  last_teacher_update_at   DATETIME     NULL COMMENT '老师最近反馈时间',
  customer_complaint       VARCHAR(16)  NULL COMMENT '是否投诉：是/否',
  ...
  submitted_expected_at    DATETIME     NULL COMMENT 'Submitted预计产出',
  with_editor_expected_at  DATETIME     NULL COMMENT 'With Editor预计产出',
  under_review_expected_at DATETIME     NULL COMMENT 'Under Review预计产出',
  revision_expected_at     DATETIME     NULL COMMENT 'Revision预计产出',
  accepted_expected_at     DATETIME     NULL COMMENT 'Accepted预计产出',
  proofing_expected_at     DATETIME     NULL COMMENT 'Proofing预计产出',
  online_expected_at       DATETIME     NULL COMMENT 'Online预计产出',
  indexed_expected_at      DATETIME     NULL COMMENT 'Indexed预计产出',
  ...
  revision_due_at          DATETIME     NULL COMMENT '返修截止时间',
...
CREATE TABLE IF NOT EXISTS order_follow_records (
  ...
  remind_stage   VARCHAR(32) NULL COMMENT '提醒阶段',
...
CREATE TABLE IF NOT EXISTS post_metrics (
  ...
  collected_at DATETIME     NOT NULL                  COMMENT '指标采集时间（精确到秒）',
  likes        BIGINT       NOT NULL DEFAULT 0         COMMENT '点赞数',
  comments     BIGINT       NOT NULL DEFAULT 0         COMMENT '评论数',
  favorites    BIGINT       NOT NULL DEFAULT 0         COMMENT '收藏数',
  shares       BIGINT       NOT NULL DEFAULT 0         COMMENT '分享数',
