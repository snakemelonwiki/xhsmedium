...
export default function AdminTeacherOrderTypesPage() {
  ...
  async function load(kw = keyword, nextPage = page, nextPageSize = pageSize) {
    setLoading(true);
    try {
      const data = await apiClient.get<PagedResult<Item>>('/teacher-order-types', {
  ...
  async function handleSubmit() {
    ...
    try {
      if (editing) {
        await apiClient.request(`/teacher-order-types/${editing.id}`, { method: 'PUT', body: values });
        message.success('已更新');
      } else {
        await apiClient.post('/teacher-order-types', values);
        message.success('已添加');
      }
      setModalOpen(false);
      void load();
  ...
  async function handleDelete(id: number) {
    try {
      await apiClient.delete(`/teacher-order-types/${id}`);
      message.success('已删除');
      void load();
    } catch (err: any) {
  ...
  const columns: ColumnsType<Item> = [
    { title: '接单类型名称', dataIndex: 'name', width: 220, ellipsis: true },
    { title: '创建时间', dataIndex: 'createdAt', width: 170, render: (v: string) => formatDateTime(v) },
    {
      ...
      render: (_: unknown, record: Item) => (
        <Space size={0} wrap={false}>
          ...
          <Popconfirm
            title="确定删除？"
            description="如果该接单类型已被老师使用，将无法删除"
  ...
  return (
    <Space direction="vertical" size={16} className="page-stack">
      <div className="toolbar-row">
        <div>
          <Typography.Title level={2}>接单类型管理</Typography.Title>
          <Typography.Paragraph type="secondary">维护老师可选的接单类型列表，老师在教务端编辑时可多选。</Typography.Paragraph>
        </div>
        <Space wrap>
          <Input
            placeholder="搜索名称"
          ...
          <Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>新增接单类型</Button>
        </Space>
      </div>
      <Card>
        <Table<Item>
          ...
          locale={{ emptyText: <Empty description="暂无接单类型" /> }}
          pagination={{
            current: page,
            pageSize,
            total,
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="名称" rules={[{ required: true, message: '请输入名称' }]}>
            <Input placeholder="如：SCI期刊、EI会议、普刊" />
          </Form.Item>
        </Form>
      </Modal>
    </Space>
